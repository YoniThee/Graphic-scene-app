...
public class Camera {
    ...
    public Camera renderImage(){
        ...
        List<rayColor> TempTotalList = new ArrayList<>();
        int threadsCount = 3;
        ...
        if(AntiAlaising && SuperSimple) {
           ...
           Pixel.waitToFinish();
            ...
            while (threadsCount-- > 0) {
                new Thread(() -> {
                    for (Pixel pixel = new Pixel(); pixel.nextPixel(); Pixel.pixelDone())
                        castBeam(constructBeam(nX, nY, pixel.col, pixel.row,divide),TempTotalList
                               ,0,pixel);
                }).start();
            }
            Pixel.waitToFinish();
       ...
       /* Color endColor = new Color(0,0,0);
        ...
        if(SuperSimple) {
            ...
            for (var rayColor : beam) {
                ...
                {
                    Totalbeam.add(rayColor);
                }
                else if (depth < 4 && rayColor.color.isChange(beam.get(2).color)) {
                    depth++;
