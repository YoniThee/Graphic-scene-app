...
class TriangleTest {
    ...
    /**
     * Test method for {@link geometries.Triangle#findGeoIntsersections(primitives.Ray)}.///////////////////////////
     */
    @Test
    ...
    public void findIntersections()
    {
        ...
        assertEquals( List.of(new Point(1,2,0.5)),
                triangle.findGeoIntsersections(new Ray(new Point(1,3,0.5),new Vector(0,-1,0))),
                "A point inside the triangle");

        // TC02: A point in front of one of the ribs
        assertNull(triangle.findGeoIntsersections(new Ray(new Point(0,2,0.5),new Vector(0,1,0.25))),"A point in front of one of the ribs");

        // TC03: A point in front of the vertex
        assertNull(triangle.findGeoIntsersections(new Ray(new Point(1,1,1),new Vector(0,1,0))),"A point in front of the vertex");
        ...
        try {
            assertNull(triangle.findGeoIntsersections(new Ray(new Point(1.5, 2, 0.5), new Vector(0, 1, 0))),
                    "A Point on the rib");
        }
        ...
        try {
        assertNull(triangle.findGeoIntsersections(new Ray(new Point(1,2,1),new Vector(0,1,0))),
        ...
        try {
        assertNull(triangle.findGeoIntsersections(new Ray(new Point(0,2,2),new Vector(-1,0,1))),"A point on the continuation of one of the ribs");
        }
        catch (IllegalArgumentException TheZeroVector){}
