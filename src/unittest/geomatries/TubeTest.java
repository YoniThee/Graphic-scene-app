...
class TubeTest {
    ...
    void testFindIntsersections() {
        ...
        List<Point> result = tube.findGeoIntsersections(new Ray(new Point(0,-4,2),new Vector(0,8,0)));
        ...
        result = tube.findGeoIntsersections(new Ray(new Point(0,-4,0),new Vector(0,8,2)));
        assertEquals(result.size(),2,"wrong number of points");
        ...
        assertEquals(tube.findGeoIntsersections(new Ray(new Point(0,-1,0),new Vector(0,1,3))),
                List.of(new Point(0,-2/3d,1)),"wrong intersect point");
        //TC04: No intersect points(0 point)
        assertNull(tube.findGeoIntsersections(new Ray(new Point(0,4,1),new Vector(0,1,3)))
                ,"wrong intersect point");

        // =============== Boundary Values Tests ==================
        //**** Group: ray from inside
        //TC10: Ray from inside intersect the base(1 point)
        assertEquals(tube.findGeoIntsersections(new Ray(new Point(0,1,2),new Vector(0,-1,-1))),
                List.of(new Point(0,0,1)),"wrong intersect point from inside to base");

        //TC11: Ray from inside intersect the side(1 point)
        assertEquals(tube.findGeoIntsersections(new Ray(new Point(0,1,2),new Vector(-2.5,0,0))),
                List.of(new Point(-2.5,1,2)),"wrong intersect point from inside to side");

        //**** Group: ray on tangent
        //TC12: Ray on tangent(0 point)
        assertNull(tube.findGeoIntsersections(new Ray(new Point(2.5,1,1),new Vector(0,1,3))),
                "wrong intersect point");
        //TC13: Ray on ths base(0 point)
        assertNull(tube.findGeoIntsersections(new Ray(new Point(2.5,1,1),new Vector(2,0,0))),
                "wrong intersect point");
    }
}